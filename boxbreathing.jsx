...
export default function BoxBreathing() {
  ...
  const [prepSecondsLeft, setPrepSecondsLeft] = useState(4);
  ...
  }, [running, prepping, prepStep, phaseIndex, advancePhase, soundOn, playTick]);
  ...
  const isPrepExhale = prepping && prepStep === 0;
  const isPrepHold = prepping && prepStep === 1;

  const activeLabel = prepping
    ? (isPrepExhale ? "吐いて" : "止めて")
    : phase.label;
  const activeSub = prepping
    ? (isPrepExhale ? "Exhale" : "Hold")
    : phase.sub;
  const activeSeconds = prepping ? prepSecondsLeft : secondsLeft;

  const highlightKey = !running
    ? null
    : prepping
    ? (isPrepExhale ? "exhale" : "hold2")
    : phase.key;
  ...
  let ballTarget;
  if (running && prepping) {
    ballTarget = isPrepExhale ? BOX_CORNERS[3] : BOX_CORNERS[0];
  } else if (running) {
    ballTarget = BOX_CORNERS[(phaseIndex + 1) % 4];
  } else {
    ballTarget = BOX_CORNERS[phaseIndex];
  }

  const displaySet = finished
    ? targetSets
    : running && !prepping
    ? Math.min(cycles + 1, targetSets)
    : cycles;
      ...
      >
        <svg width="260" height="260" style={{ position: "absolute", top: 0, left: 0 }}>
          ...
          <line
            ...
            stroke={highlightKey === "inhale" ? "#c9a876" : "transparent"}
            strokeWidth="3"
            strokeLinecap="round"
          />
          <line
            x1="20" y1="20" x2="240" y2="20"
            stroke={highlightKey === "hold1" ? "#c9a876" : "transparent"}
            strokeWidth="3"
            strokeLinecap="round"
          />
          <line
            x1="240" y1="20" x2="240" y2="240"
            stroke={highlightKey === "exhale" ? "#c9a876" : "transparent"}
            strokeWidth="3"
            strokeLinecap="round"
          />
          <line
            x1="240" y1="240" x2="20" y2="240"
            stroke={highlightKey === "hold2" ? "#c9a876" : "transparent"}
            strokeWidth="3"
            strokeLinecap="round"
          />
        </svg>
        ...
        >
          {finished ? (
            ...
            <>
              {prepping && (
                <div style={{ fontSize: "11px", color: "#7d8bb0", letterSpacing: "0.1em", marginBottom: "2px" }}>
                  準備
                </div>
              )}
              <div
